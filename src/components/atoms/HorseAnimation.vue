...
const resolvedHex = computed(() => {
  const key = props.colorName.trim().toLowerCase();
  return HORSE_COLOR_MAP[key] ?? HORSE_COLOR_MAP.charcoal;
});

const horseStyleVars = computed(() => ({
  '--color-horse': resolvedHex.value,
  '--color-horse-back': darkenHex(resolvedHex.value, 24),
  '--color-hair': darkenHex(resolvedHex.value, 12),
...
.horse *, .horse *:after, .horse *:before {
